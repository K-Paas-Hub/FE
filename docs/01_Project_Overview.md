...
- **채팅창에서 커밋 요청 시 채팅창에서 수정한 것과 관련된 파일만 커밋**
- **에러 검사 시에도 채팅창에서 수정한 관련 파일만 검사**
- **관련 없는 파일의 변경사항은 커밋에서 제외**

### 2. 테스트 관련 규칙
- **테스트 수정 시에는 테스트 파일만 수정**
- **원본 코드는 건드리지 말고 테스트만 수정**
- **테스트 실패 시 원본 코드 변경 금지**
- **테스트 커버리지 향상을 위한 원본 코드 수정 금지**
- **테스트 파일과 원본 코드는 별도로 관리**
